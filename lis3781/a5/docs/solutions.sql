...
if exists(select name from master.dbo.sysdatabases where name = N'fcs23a')
drop database fcs23a;
go

if not exists(select name from master.dbo.sysdatabases where name = N'fcs23a')
...
IF OBJECT_ID(N'dbo.product_days_of_week', N'P') IS NOT NULL
    DROP PROCEDURE dbo.product_days_of_week;
GO

CREATE PROCEDURE dbo.product_days_of_week
AS
BEGIN
    -- Because DATENAME defaults to Sunday as the first weekday, there can be an offset if desired.
    -- For now, this example simply uses tim_day directly (1 = Sunday, 2 = Monday, etc.)

    SELECT
        p.pro_name,
        p.pro_descript,
        DATENAME(WEEKDAY, tim_day + 0) AS [day_of_week]  -- +0 means no offset; adjust as needed
    FROM product p
    JOIN sale s
        ON p.pro_id = s.pro_id
    JOIN time t
        ON s.tim_id = t.tim_id
    ORDER BY t.tim_id ASC;  -- sorts numerically, rather than string return of DATENAME()
END;
GO
...
CREATE PROC dbo.product_drill_down
AS
BEGIN
    SELECT
        p.pro_name,
        p.pro_qoh,
        FORMAT(p.pro_cost, 'C', 'en-us') AS cost,
        FORMAT(p.pro_price, 'C', 'en-us') AS price,
        st.str_name,
        c.cty_name,
        st2.ste_name,
        r.reg_name
    ...
    JOIN [state] st2
        ON c.ste_id = st2.ste_id
    JOIN region r
        ON st2.reg_id = r.reg_id
...
AS
BEGIN
    -- We don’t need pay_id PK here because it’s an IDENTITY column
    INSERT INTO payment (inv_id, pay_date, pay_amt, pay_notes)
    VALUES (@inv_id_p, @pay_date_p, @pay_amt_p, @pay_notes_p);
END;
GO

-- Test or verify data before/after calling the procedure
SELECT * FROM dbo.payment;


-- Initialize variables (declare and assign values)
DECLARE 
    @inv_id_v INT = 6,
...
-- If you use parentheses, e.g. EXEC dbo.add_payment(@inv_id_v, @pay_date_v, @pay_amt_v, @pay_notes_v);
-- it will generate an error, so omit parentheses.

-- Inspect the table data after the call:
SELECT * FROM dbo.payment;
...
AS
BEGIN
	select p.per_id, p.per_fname, per_lname, i.inv_id,
		FORMAT(sum(pay_amt), 'C', 'en-us') as total_paid,
		FORMAT((inv_total - sum(pay_amt)), 'C', 'en-us') as invoice_diff
	from person p
	join dbo.customer c on c.per_id=p.per_id
	join dbo.contact ct on ct.per_cid=c.per_id
	join dbo.[order] o on o.cnt_id=ct.cnt_id
	join dbo.invoice i on i.ord_id=o.ord_id
